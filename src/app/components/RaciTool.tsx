...
export default function RaciTool({ onBack }: { onBack: () => void }) {
  const [tasksText, setTasksText] = useState(() => {
    if (typeof window !== "undefined")
      return localStorage.getItem("raci_tasks") || "";
    return "";
  });
  const [membersText, setMembersText] = useState(() => {
    if (typeof window !== "undefined")
      return localStorage.getItem("raci_members") || "";
    return "";
  });
  ...
  useEffect(() => {
    localStorage.setItem("raci_tasks", tasksText);
  }, [tasksText]);
  useEffect(() => {
    localStorage.setItem("raci_members", membersText);
  }, [membersText]);
  useEffect(() => {
    localStorage.setItem("raci_matrix", JSON.stringify(matrix));
  }, [matrix]);
  useEffect(() => {
    localStorage.setItem("raci_memberNames", JSON.stringify(memberNames));
  }, [memberNames]);
  useEffect(() => {
    localStorage.setItem("raci_memberRoles", JSON.stringify(memberRoles));
  }, [memberRoles]);
